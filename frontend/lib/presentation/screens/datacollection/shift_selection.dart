...
import 'package:mine_wadhwani/presentation/screens/health_assessment/health_assessment_prompt_page.dart';
import 'mine_info_page.dart';
...
class _ShiftSelectionPageState extends State<ShiftSelectionPage>
  ...
  // ─── Navigation ───────────────────────────────────────────────────────────
  //
  // Single place that decides which overview page to open based on the
  // selected inspection type. Add new inspection types here as the app grows.

  // void _onNext() {
  //   final shift = int.parse(_selectedShift!);
  //   final type = _selectedInspectionType!;

  //   if (type == 'Stockpile inspection') {
  ...
  //       company: widget.company,
  //     ));
  //   } else if (type == 'Haul Road inspection') {
  //     // ── NEW: routes to HaulRoadOverviewPage ──────────────────────────
  //     context.router.push(HaulRoadOverviewRoute(
  //       mineName: widget.mineName,
  //       mineType: widget.mineType,
  //       area: widget.area,
  //       shift: shift,
  ...
  //     ));
  //   } else {
  //     // All other inspection types → generic checklist flow
  ...
  void _onNext() async {
    ...
    // ── NEW: Health Assessment prompt before entering the checklist flow ──
    final healthAssessment = await Navigator.of(context).push<HealthAssessmentModel>(
      MaterialPageRoute(
        builder: (_) => HealthAssessmentPromptPage(
          mineName: widget.mineName,
          area: widget.area,
          shift: shift,
          inspectionType: type,
    ...
    // If the inspector backs out of the prompt page without choosing
    // Yes/No/Skip (e.g. hits system back), treat it as "skipped" so the
    // inspection flow still proceeds rather than getting stuck.
    final resolvedHealthAssessment =
        healthAssessment ?? HealthAssessmentModel.skipped();
    ...
    if (type == 'Stockpile inspection') {
      context.router.push(StockpileOverviewRoute(
        ...
        inspectionType: type,
        company: widget.company,
        healthAssessment: resolvedHealthAssessment,
      ));
    } else if (type == 'Haul Road inspection') {
      // ── NEW: routes to HaulRoadOverviewPage ──────────────────────
      context.router.push(HaulRoadOverviewRoute(
  ...
  Widget _buildNextButton() {
    return SizedBox(
      ...
      child: ElevatedButton(
        // ── All routing logic is now in _onNext() ──────────────────────
