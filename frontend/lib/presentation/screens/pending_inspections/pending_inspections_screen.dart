...
import 'package:mine_wadhwani/presentation/bloc/draft_bloc/draft_event.dart';
import 'package:mine_wadhwani/presentation/bloc/draft_bloc/draft_state.dart';
import 'package:mine_wadhwani/presentation/screens/haul_road/haul_road_overview_page.dart';
import 'package:mine_wadhwani/presentation/screens/stockpile/stockpile_overview_page.dart';
...
class PendingInspectionsScreen extends StatelessWidget {
  ...
  // ── Inspection type → page routing ─────────────────────────────────────────
  //
  // When resuming a draft we look at draft.inspectionType to decide which
  // overview page to push. Add new cases here as you add more inspection types.

  static Widget _overviewPageForDraft(
      BuildContext context, InspectionDraft draft) {
    final type = draft.inspectionType.toLowerCase();

    if (type == 'haul road inspection') {
      return HaulRoadOverviewPage(
        mineName: draft.mineName,
        mineType: draft.mineType,
        area: draft.area,
        shift: draft.shift,
    ...
    // Default: stockpile (and anything else that currently uses StockpileOverviewPage)
    return StockpileOverviewPage(
      mineName: draft.mineName,
      mineType: draft.mineType,
      area: draft.area,
      shift: draft.shift,
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.history_rounded, size: 52, color: primary),
          ),
          const SizedBox(height: 20),
          const Text(
            'No Pending Inspections',
  ...
  void _resumeDraft(BuildContext context, InspectionDraft draft) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => BlocProvider.value(
          value: context.read<DraftBloc>(),
          // ── Routes to the correct overview page based on inspection type ──
          child: _overviewPageForDraft(context, draft),
        ),
      ),
    );
  }
...
class _DraftCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final progress = draft.totalCount > 0
