...
//   void _onSignatureChanged() {
//     final isEmpty = _signatureControl.paths.isEmpty;
//     if (isEmpty != _signatureEmpty) {
//       setState(() => _signatureEmpty = isEmpty);
//     }
//   }
...
//   // Both buttons require name + signature — observations is optional
//   bool get _canSubmit =>
//       _nameController.text.trim().isNotEmpty && !_signatureEmpty;
...
//   Future<void> _openPreview() async {
//     // Guard — same requirement as submit
...
//             Row(
//               children: [
//                 // Preview & Export — disabled until name + signature filled
//                 Expanded(
//                   child: SizedBox(
//                     height: 50,
//                     child: OutlinedButton.icon(
...
//                       style: OutlinedButton.styleFrom(
//                         foregroundColor:
//                             _canSubmit ? _headerColor : const Color(0xFFBDBDBD),
//                         side: BorderSide(
//                           color: _canSubmit
//                               ? _headerColor
//                               : const Color(0xFFE0E3EA),
...
//                 ),
//                 const SizedBox(width: 12),
//                 // Submit & Close — disabled until name + signature filled
//                 Expanded(
//                   child: SizedBox(
//                     height: 50,
//                     child: FilledButton.icon(
...
//   // ── Section label — optional flag adds a muted "(optional)" suffix ──────────
...
class StockpileCompliancePage extends StatefulWidget {
  ...
  final Map<String, String> comments;
  final Map<String, List<String>> mediaFiles;
  final Map<String, String> actions; // ← ADD
  final String inspectorName;
  final String inspectorRole;
...
class _StockpileCompliancePageState extends State<StockpileCompliancePage> {
  ...
  void _onSignatureChanged() {
    setState(() {
      _signatureEmpty = _signatureControl.paths.isEmpty;
    });
  }
  ...
  void _validateAndSubmit() async {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('The inspection has been submitted successfully.'),
        actions: [
          FilledButton(
            onPressed: () {
              Navigator.of(context).pop();
...
Future<void> _openPreview() async {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => StockpileReportPreviewPage(
          ...
          comments: widget.comments,
          mediaFiles: widget.mediaFiles,
          actions: widget.actions, // ← ADD THIS
          inspectorName: _nameController.text.trim(),
          inspectorRole: widget.inspectorRole,
          observations: _observationsController.text.trim(),
          signatureBytes: signatureBytes,
          ...
          Expanded(
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _buildSectionLabel('GENERAL OBSERVATIONS', optional: true),
                        const SizedBox(height: 10),
                        _buildObservationsCard(),
                        const SizedBox(height: 16),
                        _buildSectionLabel('EVALUATOR NAME'),
                ...
                // Right panel — Signature (NO scroll, must be constrained)
                Expanded(
                  flex: 5,
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(12, 20, 24, 20),
                    child: Column(
                      ...
                      children: [
                        _buildSectionLabel('SIGNATURE'),
                        const SizedBox(height: 10),
                        // Signature canvas — takes all remaining height
                        Expanded(
  ...
  // ── Signature canvas — fully expanded, no Column wrapping ──────────────────
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Stack(
          fit: StackFit.expand,
          children: [
            // Signature widget — fills entire canvas
            HandSignature(
              control: _signatureControl,
              color: Colors.black,
              width: 1.0,
            ...
            // Hint — only shown when empty, never blocks touches
            if (_signatureEmpty)
              IgnorePointer(
                child: Center(
                  child: Column(
  ...
  Widget _buildObservationsCard() {
    return Theme(
      data: Theme.of(context).copyWith(
        colorScheme: Theme.of(context).colorScheme.copyWith(primary: _headerColor),
      ),
      child: TextField(
        controller: _observationsController,
        cursorColor: _headerColor,
  ...
  Widget _buildNameField() {
    return Theme(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'Full name of inspector',
          prefixIcon: const Icon(Icons.person_outline, color: _headerColor, size: 20),
          filled: true,
          fillColor: Colors.white,
          enabledBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(12),
              ...
              children: [
                    ...
                    child: OutlinedButton.icon(
                      ...
                      style: OutlinedButton.styleFrom(
                        foregroundColor:
                            _canSubmit ? _headerColor : const Color(0xFFBDBDBD),
                        side: BorderSide(
                          color: _canSubmit ? _headerColor : const Color(0xFFE0E3EA),
                          width: 1.5,
                        ),
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(14),
                ...
                Expanded(
                  child: SizedBox(
                    height: 50,
                    child: FilledButton.icon(
                      onPressed: _canSubmit && !_isSubmitting ? _validateAndSubmit : null,
                      icon: _isSubmitting
                          ? const SizedBox(
                              width: 18,
                              height: 18,
