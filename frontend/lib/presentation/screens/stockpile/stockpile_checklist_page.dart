...
//       result['__next__'] = '$nextSectionIndex';
//     }
//     return result;
//   }

//   /// Called by the back button and FINISH — returns all data with no navigation signal.
//   void _popWithResult() {
//     Navigator.of(context).pop(_buildResult());
//   }

//   /// Called by SAVE & NEXT — returns all data AND signals the next section to open.
//   void _saveAndNext() {
//     Navigator.of(context).pop(_buildResult(nextSectionIndex: _currentSectionIndex + 1));
//   }
...
//                   height: 50,
//                   child: FilledButton(
//                     // KEY FIX: FINISH pops normally; SAVE & NEXT pops with
//                     // a __next__ signal so the overview saves data AND opens
...
class _StockpileChecklistPageState extends State<StockpileChecklistPage> {
  ...
  Widget _buildQuestionCard(StockpileQuestion question) {
    switch (question.type) {
      case StockpileQuestionType.text:
        return _buildTextInputCard(question);
      case StockpileQuestionType.dropdown:
        return _buildDropdownCard(question);
      case StockpileQuestionType.datepicker:
        return _buildDatePickerCard(question);
      case StockpileQuestionType.yesNo:
        return _buildYesNoCard(question);
    }
  }
  ...
  // ─── Dropdown / tile-picker card (e.g. 1.1) ──────────────────────────────

  Widget _buildDropdownCard(StockpileQuestion question) {
  ...
  // ─── Date picker card (e.g. 1.6–1.8) ─────────────────────────────────────

  Widget _buildDatePickerCard(StockpileQuestion question) {
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              child:
                  const Icon(Icons.bolt, size: 18, color: Color(0xFFF59E0B)),
            ),
            const SizedBox(width: 10),
            Expanded(
              child: Column(
